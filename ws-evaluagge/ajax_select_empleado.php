...
$consulta_empleado = "SELECT * FROM dbo.Empleados with (nolock) WHERE TipoCargo != 4 and Nombre != 'ADMIN' and Estatus='A' ORDER BY Apellido";
$result_query = odbc_exec($conexion_sbio, $consulta_empleado);
echo '<label for="select_empresaa">Seleccion de Empleado <em class="em">*</em></label>';
echo '<select class="form-control seleccion_empleado" id="seleccion_empleado" name="seleccion_empleado" required>';
