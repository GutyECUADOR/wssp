...
<body>
        <header>
			...
			<nav>
                            <?PHP $consulta_headermenu = mysql_query("SELECT * FROM tbl_confheadermenu ORDER BY menu_id ASC");
                                ...
                                {echo '<a href="../ws-admin/'.$row['url_menu'].'"><span class="'.$row['clases_menu'].'"><span class="'.$row['clases_textmenu'].'">'.$row['nombre_menu'].'</span></span></a>';
                                }
                            ?>
                        </nav>
		</div>
    ...
    <!-- AQUI CONTENIDO PRINCIPAL-->
		...
		<form action="subirfotos.php" enctype="multipart/form-data" class="formulario" name="formulario_subirfoto" method="POST">
		<input name="input_foto" type="file" id="input_foto"  >
		<input name="cmd_guardar" type="submit" value="Subir foto" />
	</form>
	</div>
