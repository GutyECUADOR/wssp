...
$consulta_user = "SELECT  A.CODIGO as CodCliente, RUC as Cedula, B.codigo as CodigoWin, A.NOMBRE as NombreN  FROM dbo.COB_CLIENTES as A INNER JOIN dbo.ROL_EMPLEADOS as B ON A.RUC = B.cedula WHERE A.RUC='$cod_ingresado' AND CLASE='I' AND A.RUC  IN  (SELECT cedula FROM dbo.ROL_EMPLEADOS WHERE Estatus = 'A')";
...
if (odbc_num_rows($result_query_user)>=1)
    ...
    $cod_winfenix = trim(iconv("iso-8859-1", "UTF-8", odbc_result($result_query_user,"CodigoWin")));

    $rawdata = array(["Cod_Cliente"=>$cod_cli, "Nombre"=>$user_name, "Cedula"=>$user_ci, "CodigoWinF"=>$cod_winfenix ]);
